...
/***************************************************************************/
BOOL CScreeShot::MakePNG(HDC hDC, RECT rect, LPCWSTR strFilePath)
{
	BITMAP bmp;
	PBITMAPINFO pbmi;
	...
	std::wstring szBMPFilename = std::wstring(strFilePath).substr(0, std::wstring(strFilePath).length() - 3) + L"bmp";
	...
	// Create the .BMP file. 
	hfile = CreateFile(szBMPFilename.c_str(),
	...
	//转换成PNG
	if (!BMptoPNG(szBMPFilename.c_str(), strFilePath))
	{
		DeleteFile(szBMPFilename.c_str());
		return FALSE;
	}
...
// //转换BMP文件为PNG文件            
BOOL CScreeShot::BMptoPNG(LPCWSTR StrBMp, LPCWSTR StrPNG)
{
	CLSID encoderClsid;
	Status stat;
	Image* image = NULL;
...
//	功能描述:		转换PNG文件为BMP文件      
BOOL CScreeShot::PNGtoBMp(LPCWSTR StrPNG, LPCWSTR StrBMp)
{
	CLSID encoderClsid;
	Status stat;
	Image* pImage;
...
BOOL CScreeShot::GetEncoderClsid(WCHAR* pFormat, CLSID* pClsid)
{
	UINT num = 0, size = 0;
